...
def transform_image(image, im_size, device):
    transform = transforms.Compose([
        transforms.ToPILImage(),
        transforms.Resize([im_size, im_size]),
        transforms.ToTensor(),
    ...
    transformed_image = transform(image)[0:3, :, :].unsqueeze(0).to(device)
    return transformed_image

def predict(model, image_data, im_size, device):
    transformed_image = transform_image(image_data, im_size, device)
    with torch.no_grad():
        output = model(transformed_image)
        SMScore = nn.Softmax(dim=1)(output).detach().cpu().numpy()[:, 1]
    return SMScore[0]
...
def main():
    ...
    while input_rtsp.isOpened():
        ...
        score = predict(model, frame, im_size, device)
